...
if("data.table" %in% installed.packages()  && packageVersion("data.table")  >= "1.9.5" ) {
...
cleanData <- function(data) {
        #data$EVTYPE <- sapply(data$EVTYPE, changeDescriptiveName)
        clean.data <- data %>%
...
subset(clean.data, EVTYPE %in% top_n(select(summary.dmg.data ,1 ),3))

sum(clean.data$EVTYPE %in% top_n(select(summary.dmg.data ,1 ),3))
# plot 
...
source("RepData_PeerAssessment2/multiplot.R")
